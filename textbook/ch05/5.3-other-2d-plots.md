# 5.3 다른 종류의 2차원 그래프 (Other Types of Two-Dimensional Plots)
...
> ⚠️ **함정** — `plot`으로 이미 그린 뒤 `semilogy`를 부르면 새 그래프로 덮어쓰인다. 선은 그대로 두고 **기존 축의 눈금만** 바꾸고 싶다면 축 객체의 속성을 직접 고친다.
>
> ```matlab
> ax = gca;
> ax.YScale = "log";
